...
library(plyr)       # Data wrangling - load before tideyverse/dplyr!
...
```{r}
roundmean <- function(x){round(mean(x, na.rm=TRUE), 0)}

# dat_clean %>%
#   group_by(id) %>%
#   summarise(across(edu, ~ roundmean(.x))) %>% select(edu) %>% unique()

dat_clean %<>% 
  group_by(id) %>% 
  mutate(across(all_of(vars_fixed), ~ roundmean(.x))) %>% 
  ungroup()
